...
//      float newDiff = Math.abs(screenResolution.x * 1.0f / newY - screenResolution.y * 1.0f / newX);
	...
	private void setZoom(Camera.Parameters parameters) {
		...
		if (null != takingPictureZoomMaxString) {
			try {
		...
		if (null != motZoomValuesString) {
			tenDesiredZoom = findBestMotZoomValue(motZoomValuesString, tenDesiredZoom);
		}
		
		String motZoomStepString = parameters.get("mot-zoom-step");
		if (null != motZoomStepString) {
			try {
		...
		if (null != maxZoomString || null != motZoomValuesString) {
