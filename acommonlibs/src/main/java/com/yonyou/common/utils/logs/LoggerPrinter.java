...
final class LoggerPrinter implements Printer {
	...
	private static String DEFAULT_TAG = "mmmm";
	/**
	...
	public LoggerPrinter(String tag) {
		DEFAULT_TAG = tag;
		init(DEFAULT_TAG);
	}
	
	public LoggerPrinter(String tag, int methodNum) {
		DEFAULT_TAG = tag;
		// 大于5个堆栈信息的时候,不输出,防止内存溢出
		if (methodNum > 5) {
			settings.setLogLevel(LogLevel.NONE);
		} else {
			settings.setMethodCount(methodNum);
		}
		init(DEFAULT_TAG);
	}
	...
	private int getStackOffset(StackTraceElement[] trace) {
		for (int i = MIN_STACK_OFFSET; i < trace.length; i++) {
			...
			if (!name.equals(LoggerPrinter.class.getName()) && !name.equals(LogerNcc.class.getName())) {
				return --i;
			}
		}
		return -1;
