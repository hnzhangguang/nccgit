...
public class ImageRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
	...
	private class ImageViewHolder extends RecyclerView.ViewHolder {
		...
		void bind(final int position) {
				...
				public void onClick(View v) {
					...
					if (cbCheck.isChecked() && mSelectedImages.size() >= selectLimit) {
						Toast.makeText(mActivity.getApplicationContext(),
							  mActivity.getString(R.string.album_ip_select_limit, selectLimit),
			...
			imagePicker.getImageLoader().displayImage(mActivity, imageItem.path, ivThumb, mImageSize,
				  mImageSize); //显示图片
		}
	...
	private class CameraViewHolder extends RecyclerView.ViewHolder {
		...
		void bindCamera() {
			mItemView.setLayoutParams(new AbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT
				  , mImageSize)); //让图片是个正方形
