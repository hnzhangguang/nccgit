...
public class CommonScanActivity
	  extends Activity
	...
	private void setScanView(Intent intent) {
		...
		long loopWaitTime = intent.getIntExtra("loopWaitTime", 1);
		...
		this.singleThreadPool.execute(new Runnable() {
			public void run() {
				try {
					...
					CommonScanActivity.this.runOnUiThread(new Runnable() {
						public void run() {
							CommonScanActivity.this.mZBarView.startSpot();
						}
					});
