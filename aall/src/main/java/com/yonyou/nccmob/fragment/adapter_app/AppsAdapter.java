...
public class AppsAdapter extends RecyclerView.Adapter {
    ...
    public int getItemViewType(int position) {
        ...
        return app.getIsCat().equals("true") ? type_group : type_item;
    }
    ...
    private void displayItem(RecyclerView.ViewHolder holder, final int position) {
        final AppInfo model = list.get(position);
        if (getItemViewType(position) == type_group) {
            ItemGroup itemGroup = (ItemGroup) holder;
            itemGroup.tvGroupName.setText(model.getAppname());
        } else if (getItemViewType(position) == type_item) {
            ItemHolder itemHolder = (ItemHolder) holder;
            if (model.getAppname() != null) itemHolder.tvName.setText(model.getAppname());
    ...
    public interface onItemClick {
        void onClickItemEvent(AppInfo app);
    }


    public interface OnItemDeleteListener {
        void clickItem(AppInfo model, int position);
    }
